...
def build_report(
    ...
    if competition_percentile >= 70:
        swot_threat = f'서울 상위 {100 - competition_percentile}% 수준의 높은 경쟁 밀집도'
        swot_opportunity = '경쟁이 높은 상권은 수요 검증이 된 지역임을 의미할 수 있습니다.'
    elif competition_percentile >= 40:
    ...
    positive = [f'소상공인진흥공단 데이터 기준 반경 내 {category} {competitor_count}개 확인']
    if monthly_avg_sales_amt is not None:
        ...
        positive.append(f'행정동 월평균 {category} 추정매출 {amt_str}{cnt_str} (서울시 상권분석서비스 기준)')

    return {
        '요약': (
            f'{station} 반경 {radius}m 내 {category} 업종은 '
            f'경쟁 밀집도가 {density_desc}입니다.'
            + (f' 행정동 월평균 추정매출은 {monthly_avg_sales_amt:,}원입니다.' if monthly_avg_sales_amt else '')
        ),
        '긍정_요인': positive,
        '위험_요인': [risk_density],
        '전략_제안': [
        ...
        'swot': {
            '강점': [f'반경 내 {category} 업소 분포 데이터 확보 (소상공인진흥공단 기준)'],
