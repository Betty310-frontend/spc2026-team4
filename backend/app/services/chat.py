...
- 업종 추출: 사용자 메시지에서 업종을 파악한 뒤, 아래 목록 중 가장 가까운 항목을 category로 넘겨라.
  ...
  업종 단서가 조금이라도 있으면 가장 가까운 항목으로 처리하고 절대 되묻지 마라.
  특히 "학원"만 언급된 경우 "입시·교과학원"으로 처리해라.
- 위치 제한: 위치가 서울인지 여부를 네가 직접 판단하지 마라. 역명·동네명·구명 등 어떤 위치든 항상 search_competitors를 호출해라. 서울 여부 검증은 도구의 지오코더가 담당한다. 도구가 geocode_failed를 반환할 때만 위치를 찾을 수 없다고 안내해라.
- 좌표 우선: 사용자 요청이나 도구 입력에 lat/lng가 함께 있으면 위치는 이미 확정된 것으로 간주해라. 이 경우 station 텍스트를 다시 지오코딩하지 말고, 반드시 전달된 좌표를 기준으로 search_competitors / get_population_flow를 호출해라.
...
- 업종 누락: 메시지에 업종에 대한 단서가 전혀 없을 때만 "어떤 업종을 생각하고 계신가요? (예: 카페, 음식점, 미용실 등)"라고 되물어라. 업종을 추론할 수 있는 단서가 조금이라도 있으면 가장 가까운 항목으로 처리하고 바로 search_competitors를 호출해라.
...
7. 위 요약 뒤에는 `summarize`에 포함된 `확인_질문`을 우선 참고해 1~2개의 자연스러운 확인 질문을 덧붙여라.
   예: "반경을 바꿔보고 싶으신가요?", "특정 시간대 유동인구가 더 궁금하신가요?"
8. 리포트 생성 제안은 첫 분석 응답에서 바로 하지 말고, 사용자가 질문에 답하거나 조건을 바꾼 뒤에만 하도록 해라.
...
async def stream_ui(
    ...
    try:
        ...
        # 업종이 없거나 CategoryType 정확한 값이 아닐 때 LLM으로 재분류
        _valid_categories: set[str] = set(CategoryType.__args__)  # type: ignore[attr-defined]
        if not current_category or current_category not in _valid_categories:
